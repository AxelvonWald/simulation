...
export function generateHeightmap(width: number, height: number, seed: string): number[][] {
  ...
  for (let x = 0; x < width; x++) {
    ...
    for (let y = 0; y < height; y++) {
      ...
      elevation[x][y] = Math.pow(
        (value / totalAmplitude + 1) / 2, // Normalize to 0-1
        1.5 // Makes low areas flatter and highs more dramatic
      );
      
      // Add slight random perturbation
      elevation[x][y] = Math.max(0, Math.min(1,
        elevation[x][y] + (Math.random() * 0.05 - 0.025)
      ));
    }
  }
  return elevation;
